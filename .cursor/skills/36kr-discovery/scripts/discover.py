#!/usr/bin/env python3
"""36氪发现层适配器 — gateway subNav/flow 列表 + 文章页 initialState 正文。"""
...
DEFAULT_HEADERS = {
    ...
    "Origin": BASE_URL,
    "Referer": f"{BASE_URL}/information/web_news/",
    "User-Agent": (
...
def _fetch_flow_page(*, per: int, page_event: int, page_callback: str) -> dict:
    payload = _request_json(LIST_API, data=_flow_body(per=per, page_event=page_event, page_callback=page_callback))
...
def fetch_list_page(page: int = 1, per: int = 20) -> dict:
    ...
    page_event = 0
    page_callback = ""
    result: dict = {"items": [], "page": page, "page_callback": "", "has_next_page": False, "per": per}
...
def fetch_article_detail(article_id: str) -> dict:
    ...
    return {
        ...
        "title": str(data.get("widgetTitle") or ""),
        "url": page_url,
        "published_at": _format_published_at(data.get("publishTime")),
        "author": str(data.get("author") or ""),
        "image": str(data.get("coverImageUrl") or data.get("widgetImage") or ""),
...
def main() -> int:
    parser = argparse.ArgumentParser(description="发现 36氪 最新资讯")
