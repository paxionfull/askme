...
from credential_store import cookie_satisfies_slot, get_slot_meta, upsert_credential
            ...
            while time.time() < deadline:
                ...
                if header and header != last_header:
                    ...
                    if fields_ready:
                        ...
                        token = str(meta.get("required_token") or "").strip().rstrip("=")
                        hint = (
                            f"仍为访客态，请完成扫码/手机登录（需含 {token}）"
                            if token
                            else "仍为访客态，请完成登录后再等待自动捕获"
                        )
                        _set_status(session, "waiting_login", hint)
                time.sleep(1.2)
