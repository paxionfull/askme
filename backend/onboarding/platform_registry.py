...
# hard_repair: 探测失败时先登记再修，且重探必须成功（知乎）
# register_then_repair: 先登记，失败可 soft 或 repair（Reddit）
# soft: 始终登记；探测问题仅告警（X）
...
def _zhihu_early_auth(probe: dict[str, Any]) -> str | None:
    ...
    if (
        detail.upper().startswith("ASKME_AUTH_REQUIRED")
        or "ZHIHU_COOKIE" in detail
        or "z_c0" in detail
        or not probe.get("cookie_configured")
    ):
        return (
            "ASKME_AUTH_REQUIRED:slot=zhihu 知乎接入需要登录授权："
            "请先添加含 d_c0 与 z_c0 的知乎登录 Cookie（访客态无法拉文章列表）"
        )
    return None


def _x_early_auth(probe: dict[str, Any]) -> str | None:
    if not probe.get("cookie_configured"):
        return (
            "ASKME_AUTH_REQUIRED:slot=x X 接入需要登录授权："
            "请先在设置页完成 X Cookie 授权（须含 auth_token 与 ct0）"
        )
    return None
...
PLATFORM_SPECS: dict[str, PlatformSpec] = {
    ...
    "x": PlatformSpec(
        ...
        check_early_auth=_x_early_auth,
        fail_mode="soft",
