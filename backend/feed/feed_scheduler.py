...
# 微信 list_ex 含页间限速，单独给更长超时
DEFAULT_WEIXIN_FEED_REFRESH_TIMEOUT = max(
    DEFAULT_FEED_REFRESH_TIMEOUT,
    float(os.getenv("FEED_WEIXIN_REFRESH_TIMEOUT", "90")),
)
...
class FeedSchedulerManager:
    ...
    async def _run_refresh_all(self) -> None:
        if self.is_refresh_running():
            ...
            return
        try:
            result = await self.run_refresh_all()
        ...
        *,
        ...
        async with self._refresh_lock:
            ...
            if total > 0:
                ...
                weixin_timeout = DEFAULT_WEIXIN_FEED_REFRESH_TIMEOUT

                async def refresh_one(feed: dict[str, Any]) -> tuple[str, str, bool, dict[str, Any] | None, str | None]:
                    feed_id = str(feed.get("id", ""))
                    feed_name = str(feed.get("mpName", ""))
                    is_weixin = _is_weixin_feed(feed_id)
                    slot = weixin_sem if is_weixin else gen_sem
                    timeout = weixin_timeout if is_weixin else per_feed_timeout
                    await slot.acquire()
