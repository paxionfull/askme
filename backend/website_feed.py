...
from feed_http_policy import page_delay_seconds
from html_utils import extract_article_html, html_to_text
from time_scope import calendar_scope_cutoff, days_range_label, format_duration_zh, parse_publish_time
from skill_detail import call_fetch_article_detail
...
class WebsiteFeed:
    ...
    async def refresh_feed(self, **overrides: Any) -> dict[str, Any]:
        ...
        # 默认不限制页数：一直拉到站点 has_next_page 为 false（或返回空 items）。
        # max_pages 仅支持通过 refresh_feed(max_pages=…) 显式传入，不从 REFRESH_DEFAULTS 读取。
        ...
        max_pages = int(overrides["max_pages"]) if "max_pages" in overrides else 0
        ...
        while True:
            ...
            if not self.adapter.has_next_page(payload):
                break
            # 内存切片分页（sitemap 缓存）通常 <0.2s，跳过页间 sleep；真网络分页才限速
            if fetch_elapsed >= 0.2:
