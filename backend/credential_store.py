...
from auth_signals import resolve_slot_from_url
    ...
    return [
        {
            ...
            "cookie_hint": meta["cookie_hint"],
        }
        for slot_id, meta in SLOT_DEFS.items()
    ]


def get_slot_meta(slot: str) -> dict[str, str] | None:
    meta = SLOT_DEFS.get(slot.strip().lower())
    return dict(meta) if meta else None


def ensure_slot_meta(slot: str, *, login_url: str = "", label: str = "") -> dict[str, str]:
    """已知 slot 返回定义；未知 slot 返回动态占位（供 Agent 发现的新站）。"""
    ...
    host = ""
    if login_url:
        host = urlparse(login_url).netloc.replace("www.", "")
    return {
        "label": label or host or slot_id,
        "login_url": login_url or (f"https://{host}" if host else ""),
        "cookie_hint": "粘贴该站点登录后的完整 Cookie",
        "required_token": "",
    }
...
def cookie_has_required_token(cookie: str, token: str) -> bool:
    """校验 required_token（如 web_session=）对应键存在且值非空。"""
    token_text = (token or "").strip()
    if not token_text:
        return True
    key = token_text.rstrip("=").strip()
    if not key:
        return True
...
def validate_cookie_for_slot(slot: str, cookie: str, *, probe_url: str = "") -> None:
    ...
    token = str(meta.get("required_token") or "").strip()
    if token and not cookie_has_required_token(text, token):
        label = meta.get("label") or slot
        raise ValueError(f"Cookie 缺少有效的 {token.rstrip('=')}，无法用于{label}（访客态无效）")
