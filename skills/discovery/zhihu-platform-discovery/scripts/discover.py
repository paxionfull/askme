...
def fetch_list_page(page: int = 1, per: int = 20) -> dict:
    ...
    if not user_id:
        raise ValueError("platform_accounts 缺少 account_key（知乎 user_id）")
...
def fetch_article_detail(article_id: str, **hints) -> dict:
    ...
    page_url = str(
        acc.get("posts_url")
        or acc.get("entry_url")
        or resolve_detail_url(article_id, **hints)
        or meta.get("url")
        or "https://www.zhihu.com/"
    ).strip()
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="本地调试：临时绑定 account_key（知乎 user_id）",
    )
    parser.add_argument("--user-type", type=str, default="people")
    ...
    try:
        ...
        if args.account_key.strip():
            key = args.account_key.strip()
            ut = (args.user_type or "people").strip() or "people"
            scope_account = {
                "platform": PLATFORM,
                "account_key": key,
                "user_type": ut,
                "entry_url": f"https://www.zhihu.com/{ut}/{key}",
                "posts_url": f"https://www.zhihu.com/{ut}/{key}/posts",
            }

        def _run() -> dict:
            if args.id:
                return fetch_article_detail(args.id)
        ...
        if scope_account is not None:
            with platform_account_scope(scope_account):
                output = _run()
        else:
            output = _run()
