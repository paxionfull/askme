...
| x-platform-discovery | Discovers X (Twitter) user original posts only (no replies, retweets, or quote tweets) via logged-in GraphQL (Cookie auth_token+ct0), with Nitter/syndication fallback. One skill covers all accounts; screen_name from feed_registry.platform_accounts. Use when fetching updates from x.com profiles. Requires X Cookie (auth_slot=x). | skills/discovery/x-platform-discovery/ |
