...
def _item_not_original_post(item: dict) -> bool:
    ...
    for key in ("summary", "text", "title", "raw_text"):
        value = str(item.get(key) or "").lstrip()
        lower = value.lower()
...
def _fetch_graphql_operation(
    ...
    *,
    ...
    # soft 模式只试第一个 queryId，避免 guest 404 拖慢
...
def _fetch_graphql_tweets(count: int = 40) -> list[dict]:
    ...
    return _fetch_graphql_operation(
        "UserTweets", USER_TWEETS_QUERY_IDS, count=count, guest=guest, soft=True
    )
...
def _fetch_nitter_tweets() -> list[dict]:
    ...
    for mirror in NITTER_MIRRORS:
        try:
            collected = _fetch_nitter_page(mirror, f"/{SCREEN_NAME}")
            if collected:
                return collected
        except Exception as exc:  # noqa: BLE001
            last_err = exc
